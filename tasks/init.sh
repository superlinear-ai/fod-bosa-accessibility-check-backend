...
function create_conda_env {
    ...
    log "Done!\\n"
    log "Creating conda environment accessibility-check-backend-env...\\n\\n"
    pip install --quiet conda-merge
    conda-merge environment.run.yml environment.dev.yml > environment.yml
    mamba env create --force
    rm environment.yml
    log "Done!\\n"
    log "Installing editable accessibility_check_backend into conda environment...\\n\\n"
    # shellcheck disable=SC1091
    source activate accessibility-check-backend-env
...
function install_fish {
    ...
    echo "conda activate entities-env" >> ~/.config/fish/config.fish
    log "Done!\\n"
}

function list_tasks {
    log "Local environment ready! Remember to activate your conda environment with:\\n\\n"
    log "$ conda activate accessibility-check-backend-env\\n\\n"
